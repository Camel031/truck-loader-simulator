...
st.markdown(
    ...
    /* 區塊框線 */
    .section-box { border:1px solid rgba(255,255,255,0.12); background: rgba(255,255,255,0.03); padding:14px; border-radius:12px; margin-bottom:12px; }
...
class TruckPacker:
    ...
    def _check_fit(self, item, dim_w, dim_l, dim_h, x, y, z, allow_mix_stacking):
        ...
        if z > 0:
            ...
            support_region = self.grid[gx:gx+gw, gy:gy+gl, support_z]
            if np.any(support_region == 0):
                return False
            if not allow_mix_stacking:
                if not np.all(support_region == item.id):
                    return False
        return True
...
def draw_2d_views(packer):
    ...
    ax1.annotate(LABELS['cab'], xy=(truck_w/2, 0), xytext=(truck_w/2, -15),
                 arrowprops=dict(facecolor='red', shrink=0.05), ha='center', color='red', fontweight='bold')
    ax1.annotate(LABELS['door'], xy=(truck_w/2, truck_l), xytext=(truck_w/2, truck_l+15),
    ...
    ax2.annotate(LABELS['cab_arrow'], xy=(0, truck_h/2), xytext=(-15, truck_h/2),
                 arrowprops=dict(facecolor='red', shrink=0.05), va='center', ha='right', color='red', fontweight='bold')
...
st.set_page_config(page_title="Pro Truck Loader V1", layout="wide", initial_sidebar_state="expanded")
...
with st.sidebar:
    ...
    if selected_preset != "自訂尺寸":
        ...
        if 'truck_dims' not in st.session_state:
            p_w, p_l, p_h = 220, 450, 200
        else:
            p_w, p_l, p_h = st.session_state.truck_dims

    if 'truck_dims' not in st.session_state:
        st.session_state.truck_dims = (p_w, p_l, p_h)

    # 當選擇的車型有變更時，覆寫 truck_dims 並清除對應 widget 的 state，好讓預設值更新
    if selected_preset != st.session_state.prev_preset:
        st.session_state.truck_dims = (p_w, p_l, p_h)
        for k in ["w_in", "l_in", "h_in"]:
            if k in st.session_state:
                del st.session_state[k]
        st.session_state.prev_preset = selected_preset
        set_dirty()

    truck_w = st.number_input("寬 (Width)", value=st.session_state.truck_dims[0], step=10, on_change=set_dirty, key="w_in")
    truck_l = st.number_input("長 (Length)", value=st.session_state.truck_dims[1], step=10, on_change=set_dirty, key="l_in")
    truck_h = st.number_input("高 (Height)", value=st.session_state.truck_dims[2], step=10, on_change=set_dirty, key="h_in")
...
if st.session_state.is_dirty and st.session_state.results:
    st.warning("⚠️ 資料已變更，請重新計算。", icon="⚠️")
...
with up_col:
    st.markdown("<div class='section-title'>上傳貨物清單 (CSV)</div>", unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        try:
            ...
            st.error(f"讀取失敗: {e}")
with dl_col:
    st.markdown("<div class='section-title'>下載 CSV 範本</div>", unsafe_allow_html=True)
